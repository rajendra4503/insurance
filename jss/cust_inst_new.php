...
if((isset($_REQUEST['prescriptionName']))&&(!empty($_REQUEST['prescriptionName']))){
	...
	$delete_presc_ 	= mysql_query("delete from USER_INSTRUCTION_HEADER where PlanCode = '$plancode_for_current_plan' and UserID='$userid_for_current_plan'");
	$delete_ordinary_user_inst 	= mysql_query("delete from ACTIVITES_ORDINARY_USER where PlanCode = '$plancode_for_current_plan' and UserID='$userid_for_current_plan' and SectionID='6'");
	$delete_all_prescriptions 	= mysql_query("delete from USER_INSTRUCTION_DETAILS where PlanCode = '$plancode_for_current_plan' and UserID='$userid_for_current_plan'");
	$get_last_prescription_num 	= mysql_query("select max(PrescriptionNo) from USER_INSTRUCTION_HEADER where PlanCode = '$plancode_for_current_plan' and UserID='$userid_for_current_plan'");
	...
	$insert_header_details 	= " insert into USER_INSTRUCTION_HEADER (UserID,Plancode, PrescriptionNo, PrescriptionName, DoctorsName, CreatedDate, CreatedBy) values ('$userid_for_current_plan','$plancode_for_current_plan', '$current_presc_num', '$prescription_name', '$doctor_name', now(), '$logged_userid')";
	...
	foreach ($medicineids as $ids) {
		if($ids != ""){
			...
			$start_time = mysql_real_escape_string(trim(htmlspecialchars($_REQUEST["start_time$ids"])));
  $end_time  = mysql_real_escape_string(trim(htmlspecialchars($_REQUEST["end_time$ids"])));
		...
		$insert_medicine_details = "insert into USER_INSTRUCTION_DETAILS (`UserID`, `PlanCode`, `PrescriptionNo`, `RowNo`, `MedicineName`, `InstructionTypeID`, `ThresholdLimit`, `When`, `SpecificTime`, `Instruction`, `Link`, `OriginalFileName`, `Frequency`,`FrequencyString`,`HowLong`,`HowLongType`,`IsCritical`,`ResponseRequired`,`StartFlag`,`NoOfDaysAfterPlanStarts`,`SpecificDate`,`StartTime`,`EndTime`,`CreatedDate`,`CreatedBy`) values ('$userid_for_current_plan','$plancode_for_current_plan', '$current_presc_num', '$count', '$medicinename', '$instructiontypes', '$threshold','$when','$specifictime','$instruction','$fullpath','$filename','$frequency','$frequencystring','$howlong','$howlongtype','$iscritical','$responserequired','$startflag','$numberofdaysafterplan','$specific_date','$start_time','$end_time',now(),'$logged_userid')";
				//echo $insert_medicine_details;
				$insert_header_run  	= mysql_query($insert_medicine_details);
				$count++;
		//exit;
	...
	//$specifictime = date('H:i:s',strtotime($specifictime));
	$insert_medicine_details = "insert into USER_INSTRUCTION_DETAILS (`UserID`, `PlanCode`, `PrescriptionNo`, `RowNo`, `MedicineName`, `InstructionTypeID`, `ThresholdLimit`, `When`,`Instruction`,`Link`, `OriginalFileName`, `Frequency`,`FrequencyString`,`HowLong`,`HowLongType`,`IsCritical`,`ResponseRequired`,`StartFlag`,`NoOfDaysAfterPlanStarts`,`SpecificDate`,`StartTime`,`EndTime`,`CreatedDate`,`CreatedBy`) values ('$userid_for_current_plan','$plancode_for_current_plan', '$current_presc_num', '$count', '$medicinename', '$instructiontypes','$threshold','$when','$instruction','$fullpath','$filename','$frequency','$frequencystring','$howlong','$howlongtype','$iscritical','$responserequired','$startflag','$numberofdaysafterplan','$specific_date','$start_time','$end_time',now(),'$logged_userid')";
    ...
    <div class="col-sm-2 paddingrl0"  style="display:none;" id="sidebargrid">
      ...
      <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 paddingrl0" align="right"  id="plantitle"><button type="button" class="btns" align="right" id="finished_adding"><img src="images/finishAdd.png" style="height:20px;width:auto;margin-bottom:3px;">&nbsp;FINISH CUSTOMIZING</button></div>
    	...
    	<div class="navbar navbar-default" role="navigation" style="height: 100%;background-color:#004F35;position:fixed;width:16.7%;">
    		...
    		<h6 style="font-family:Freestyle;font-size:33px;margin-top:-1px;letter-spacing:1px;color:#f2bd43;background-color:#000;">Instruction List</h6>
    		<div class="sidebarheadings">Master Instructions :</div>
			    		...
			    		<?php
			    		$get_plan_prescriptions 		= "select t1.PrescriptionNo,t1.PlanCode, t1.PrescriptionName, t1.DoctorsName, t1.CreatedDate from INSTRUCTION_HEADER as t1, PLAN_HEADER as t2 where t1.PlanCode = t2.PlanCode and t2.MerchantID = '$logged_merchantid' and t2.PlanStatus='A'";
		    		...
		    		<div class="sidebarheadings" style="margin-top: -13px;">Assigned Instructions :</div>
			    		...
			    		<?php
			    		$get_plan_prescriptions 		= "select t1.UserID,t1.PrescriptionNo,t1.PlanCode, t1.PrescriptionName, t1.DoctorsName, t1.CreatedDate from USER_INSTRUCTION_HEADER as t1, USER_PLAN_HEADER as t2 where t1.PlanCode = t2.PlanCode and t2.MerchantID = '$logged_merchantid' and t1.UserID = t2.UserID";
		    		$get_plan_prescriptions_run 	= mysql_query($get_plan_prescriptions);
		    		$get_plan_prescriptions_count 	= mysql_num_rows($get_plan_prescriptions_run);
		    		if($get_plan_prescriptions_count > 0){
			    		while ($get_presc_row = mysql_fetch_array($get_plan_prescriptions_run)) {
        ...
        var review_notes           = $('#review_notes').val();
			...
			$('#addItemButton, #getmedications').click(function(){
				...
				if(medicationcount == 0){
					$.ajax({
						type        : "GET",
						url			: "instructionCustomizePage.php",
						dataType	: "html",
						success	: function (response)
						{
			...
			$(document).on('change', '.prescriptionradio', function () {
				...
				for (i = 0; i < medicationcount; ++i) {
				 	...
				 	if(!medicinename == ""){
				 		...
				 		//bootbox.alert(wheninput);
				 		if(wheninput == 0){
				 			bootbox.alert("Please select the Instruction dosage");
				 			$('.bootbox').on('hidden.bs.modal', function() {
				 				$('#when'+result[i]).focus();
				 			});
				 			return false;
