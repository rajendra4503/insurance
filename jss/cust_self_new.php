...
if((isset($_REQUEST['hidden_value']))&&(!empty($_REQUEST['hidden_value']))){
	...
	if($selftestcount > '0'){
		...
		$delete_ordinary_user_self_test 	= mysql_query("delete from ACTIVITES_ORDINARY_USER where PlanCode = '$plancode_for_current_plan' and UserID='$userid_for_current_plan' and SectionID='3-1'");
	...
	foreach ($selfttestids as $ids) {
		if($ids != ""){
			...
			$insert_selfttest_details = "insert into USER_SELF_TEST_DETAILS (`UserID`,`PlanCode`,`SelfTestID`,`RowNo`,`MedicalTestID`,`TestName`,`DoctorsName`,`TestDescription`, `InstructionID`, `Frequency`,`FrequencyString`,`HowLong`,`HowLongType`,`Link`, `OriginalFileName`,`ResponseRequired`,`StartFlag`,`NoOfDaysAfterPlanStarts`,`SpecificDate`,`CreatedDate`,`CreatedBy`) values ('$userid_for_current_plan','$plancode_for_current_plan', '$current_selftest_num', '$count','$selftestName', '$selftestOther','$doctor_name','$selftestdesc','$when','$frequency','$frequencystring','$howlong','$howlongtype','$fullpath','$filename','$responserequired','$startflag','$numberofdaysafterplan','$specific_date', now(), '$logged_userid');";
	...
	//$update_plan_header = mysql_query("update PLAN_HEADER set PlanStatus = 'A' where Plancode='$plancode_for_current_plan'");
  		...
  		header("Location:cust_self_new.php");
}
?>
<!DOCTYPE html>
