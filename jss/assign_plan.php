...
<html lang="en" class="no-js">
	...
	<div id="planpiper_wrapper"class="fullheight">
		 ...
		 <div class="col-sm-8 paddingrl0" id="content_wrapper">
		 	...
		 	<div id="pageheading">Select a Plan from Plan List on the right & Assign it to a Patient</div>
			</div>
		 	<section>
	...
	foreach ($userarray as $user) {
		...
		if($assigned_to_user > 0){
			...
			$get_instruction_details = mysql_query("select `PlanCode`, `PrescriptionNo`, `RowNo`, `MedicineName`, `When`, `SpecificTime`, `InstructionTypeID`, `Instruction`,`Link`, `OriginalFileName`, `Frequency`, `FrequencyString`, `HowLong`, `HowLongType`, `IsCritical`, `ResponseRequired`, `StartFlag`, `NoOfDaysAfterPlanStarts`, `ThresholdLimit`, `SpecificDate`,`StartTime`,`EndTime`,`CreatedDate`, `CreatedBy` from INSTRUCTION_DETAILS where `PlanCode` = '$selectedplancode'");
			...
			if($med_details_count > 0){
				while ($instruction_details = mysql_fetch_array($get_instruction_details)) {
					...
					$user_id_StartTime  	= $instruction_details['StartTime'];
					$user_id_EndTime  		= $instruction_details['EndTime'];

					//INSERT INTO USER INSTRUCTION DETAILS

					$insert_to_user_med_details = mysql_query("insert into USER_INSTRUCTION_DETAILS (`UserID`, `PlanCode`, `PrescriptionNo`, `RowNo`, `MedicineName`, `When`, `SpecificTime`, `InstructionTypeID`, `Instruction`,`Link`, `OriginalFileName`, `Frequency`, `FrequencyString`, `HowLong`, `HowLongType`, `IsCritical`, `ResponseRequired`, `StartFlag`, `NoOfDaysAfterPlanStarts`, `ThresholdLimit`, `SpecificDate`,`StartTime`,`EndTime`, `CreatedDate`,`CreatedBy`) values ('$user', '$user_id_plan_code', '$user_id_presc_no', '$user_id_row_no', '$user_id_med_name', '$user_id_when','$user_id_sptime', '$user_id_insttid', '$user_id_instruction','$user_id_link', '$user_id_ofn','$user_id_frequency','$user_id_freq_string','$user_id_how_long','$user_id_howlong_type','$user_id_iscritical','$user_id_respreqd', '$user_id_startflag', '$user_id_no_days', '$user_id_threshold', '$user_id_specific_date','$user_id_StartTime','$user_id_EndTime','$user_id_created_date','$user_id_created_by')");
