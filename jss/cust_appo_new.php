...
if((isset($_REQUEST['hidden_value']))&&(!empty($_REQUEST['hidden_value']))){
	...
	$delete_user_header = mysql_query("delete from USER_APPOINTMENT_HEADER where  PlanCode = '$plancode_for_current_plan' and UserID='$userid_for_current_plan'");
	$delete_ordinary_user_inst 	= mysql_query("delete from ACTIVITES_ORDINARY_USER where PlanCode = '$plancode_for_current_plan' and UserID='$userid_for_current_plan' and SectionID='2'");
	$delete_user_details = mysql_query("delete from USER_APPOINTMENT_DETAILS where  PlanCode = '$plancode_for_current_plan' and UserID='$userid_for_current_plan'");	
	}
	...
	foreach ($appointmentids as $ids) {
		if($ids != ""){
			...
			$insert_appointment_details = "insert into USER_APPOINTMENT_DETAILS (`UserID`,`PlanCode`, `AppointmentDate`, `AppointmentTime`, `AppointmentDuration`, `AppointmentPlace`,  `AppointmentShortName`, `DoctorsName`, `AppointmentRequirements`, `CreatedDate`, `CreatedBy`) values ('$userid_for_current_plan','$plancode_for_current_plan', '$appointment_date', '$appointment_time','$appointmentduration', '$appointment_venue', '$appointment_name','$doctor_name','$appointment_reqs', now(), '$logged_userid');";
	...
	foreach ($datearray as $date) {
		$insert_appointment_header = "insert into USER_APPOINTMENT_HEADER (`UserID`,`PlanCode`, `AppointmentDate`, `CreatedDate`, `CreatedBy`) values ('$userid_for_current_plan','$plancode_for_current_plan', '$date', now(), '$logged_userid')";
    ...
    <div class="col-sm-2 paddingrl0"  style="display:none;" id="sidebargrid">
      ...
      <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 paddingrl0" align="right"  id="plantitle"><button type="button" class="btns" align="right" id="finished_adding"><img src="images/finishAdd.png" style="height:20px;width:auto;margin-bottom:3px;">&nbsp;FINISH CUSTOMIZING</button></div>
    	...
    	<div class="navbar navbar-default" role="navigation" style="height: 100%;background-color:#004F35;position:fixed;width:16.7%;">
    		...
    		<h6 style="font-family:Freestyle;font-size:33px;margin-top:-1px;letter-spacing:1px;color:#f2bd43;background-color:#000;">Appointment List</h6>
    		<div class="sidebarheadings">Master Appointments :</div>
    			<div class="panel-group masterplanactivities" id="accordion1" role="tablist" aria-multiselectable="true" style="max-height:250px;overflow:scroll;overflow-x:hidden;">
			    		<?php
			    		$get_plan_appointments 		= "select t1.PlanCode, t1.AppointmentShortName, t1.DoctorsName, t1.AppointmentDate, t1.AppointmentTime from APPOINTMENT_DETAILS as t1, PLAN_HEADER as t2 where t1.PlanCode = t2.PlanCode and t2.MerchantID = '$logged_merchantid' and t2.PlanStatus='A'";
		    		...
		    		<div class="sidebarheadings" style="margin-top: -13px;">Assigned Appointments :</div>
			    		...
			    		<?php
			    		$get_plan_appointments 		= "select  t1.UserID, t1.PlanCode, t1.AppointmentShortName, t1.DoctorsName, t1.AppointmentDate, t1.AppointmentTime from USER_APPOINTMENT_DETAILS as t1, USER_PLAN_HEADER as t2 where t1.PlanCode = t2.PlanCode and t2.MerchantID = '$logged_merchantid' and t1.UserID = t2.UserID";
          ...
          //alert(dataString);
			...
			$('#addItemButton, #getappointments').click(function(){
				...
				if(appointmentcount == 0){
					$.ajax({
						type        : "GET",
						url			: "appointmentCustomizePage.php",
						dataType	: "html",
						success	: function (response)
						{ 
