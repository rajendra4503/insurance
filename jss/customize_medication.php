...
/*FUNCTION TO CALCULATE DATE WEEK OR MONTH DIFFERENCE*/
function diff($from,$to,$howlongtype)
{
...
$get_plan_sync_date 	= "select UserStartOrUpdateDateTime from USER_PLAN_HEADER where UserID='$assigned_to_user'
							and PlanCode='$plan_to_customize' and MerchantID='$logged_merchantid'";
...
$get_max_rowno = mysql_query("select max(RowNo) from USER_MEDICATION_DETAILS where `PlanCode` = '$plan_to_customize' and UserID='$assigned_to_user' and `PrescriptionNo` = '$presc_edit_id'");
...
if((isset($_REQUEST['prescriptionName']))&&(!empty($_REQUEST['prescriptionName']))){
	...
	$update_query 				= "update USER_MEDICATION_HEADER set `PrescriptionName` = '$prescription_name', `DoctorsName` = '$doctor_name', `UpdatedBy` = '$logged_userid' where `PlanCode` = '$plan_to_customize' and `PrescriptionNo` = '$presc_edit_id' and UserID='$assigned_to_user'";
	...
	foreach ($existingids as $delid) {
		if($delid != ""){
			if(!in_array($delid, $usedmedids)){
				$delete_medicine_details = mysql_query("delete from USER_MEDICATION_DETAILS where `PlanCode` = '$plan_to_customize' and `PrescriptionNo` = '$presc_edit_id' and `RowNo` = '$delid' and UserID='$assigned_to_user'");
			}
		}
	}
	$rand = mt_rand(1,999);
  $update_header = mysql_query("update USER_PLAN_HEADER set PlanStatus = '$rand' where PlanCode='$plan_to_customize' and UserID = '$assigned_to_user'");
	header("location:customize_medication.php");
}
...
<html lang="en" class="no-js">
    <head>
		...
		<title>Plan Piper | Customize Medication</title>
...
<button type="button" class="btns" align="right" id="finished_adding"><img src="images/finishAdd.png" style="height:20px;width:auto;margin-bottom:3px;">&nbsp;FINISH CUSTOMIZING</button></div>
			...
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				<ul class="nav nav-pills nav-justified">
					...
					echo $modules;
					?>
					<!--<li role="presentation" class="active navbartoptabs"><a href="customize_medication.php">MEDICATION</a></li>
    	...
    	<div class="navbar navbar-default" role="navigation" style="height: 100%;background-color:#004F35;position:fixed;width:16.7%;">
    		<div id="listBar">
		    	...
		    	<button type="button" id="addItemButton" class="btns" style="display:none;">Add A Prescription<img src="images/addItem.png"></button>
		    		<?php 
		    		$get_plan_prescriptions 		= "select PrescriptionNo, PrescriptionName, DoctorsName, CreatedDate from USER_MEDICATION_HEADER where PlanCode = '$plan_to_customize' and UserID='$assigned_to_user'";
	    	...
	    	<div id="dynamicPagePlusActionBar">
		    	...
		    	<?php 
		    		if(isset($presc_edit_id)){
		    			$get_medheader_to_edit = mysql_query("select PrescriptionName, DoctorsName from USER_MEDICATION_HEADER where PrescriptionNo = '$presc_edit_id' and PlanCode = '$plan_to_customize' and UserID='$assigned_to_user'");
		    	...
		    	?>
				   ...
				   <?php 
				   	if(isset($presc_edit_id)){
				   		...
				   		$med_count = 0;
				   		$med_count_string = "";
				   		$get_meddetails_to_edit = mysql_query("select `RowNo`, `MedicineName`, `When`, `Instruction`,`Link`, `Frequency`, `FrequencyString`, `Howlong`, `HowLongType`, `IsCritical`, `ResponseRequired`, `StartFlag`, `NoOfDaysAfterPlanStarts`, `SpecificDate`, `SpecificTime` from USER_MEDICATION_DETAILS where PrescriptionNo = '$presc_edit_id' and PlanCode = '$plan_to_customize' and UserID='$assigned_to_user'");
		...
		<tr>
          ...
          <?php
          /*if($plan_synced_date!="" && $plan_synced_date!='1970-01-01')
          {
          	//echo 123;exit;
			//$howlongtype      = "Days";
			//$plan_synced_date = "2015-01-01";
			$current_date     	= date('Y-m-d');

			$howlong 		= diff($plan_synced_date,$current_date,$howlongtype);
          }*/
          //echo $howlong;exit;
          ?>
    ...
    <!--END OF SHOW MONTH DAY PICKER MODAL WINDOW-->
		...
		$('.editmedicationbuttons').click(function(){
			...
			window.location.href = "customize_medication.php?id="+prescid;
		});
	...
	//echo "Check:".$plan_to_customize;exit;
	?>
</html>