...
//GET PLAN SYNCED DATE FROM DATABASE AND ACCORDINGLY SUBSTRACT DURATION (days,weeks,months) so that when any activity is updated by merchant
//and user updates the same in mobile database, the number of days or weeks or months calculation is done properly
$get_plan_sync_date 	= "select UserStartOrUpdateDateTime from USER_PLAN_HEADER where UserID='$logged_userid'
							and PlanCode='$current_created_plancode' and MerchantID='$logged_merchantid'";
//echo $get_plan_sync_date;exit;
$get_plan_sync_date_qry	= mysql_query($get_plan_sync_date);
$get_plan_sync_count 	= mysql_num_rows($get_plan_sync_date_qry);
if($get_plan_sync_count)
{
	$row= mysql_fetch_array($get_plan_sync_date_qry);
	$plan_synced_date = date('Y-m-d',strtotime($row['UserStartOrUpdateDateTime']));
}
//echo $plan_synced_time;exit;
...
$get_max_rowno = mysql_query("select max(RowNo) from MEDICATION_DETAILS where `PlanCode` = '$current_created_plancode' and `PrescriptionNo` = '$presc_edit_id'");
...
if((isset($_REQUEST['prescriptionName']))&&(!empty($_REQUEST['prescriptionName']))){
	...
	$update_query 				= "update MEDICATION_HEADER set `PrescriptionName` = '$prescription_name', `DoctorsName` = '$doctor_name', `UpdatedBy` = '$logged_userid' WHERE `PlanCode` = '$current_created_plancode' AND `PrescriptionNo` = '$presc_edit_id'";
	...
	foreach ($usedmedids as $ids) {
				...
				if(in_array($ids, $existingids)){
					...
					$update_query = "update `MEDICATION_DETAILS` set `MedicineName` = '$medicinename', `When` = '$when', `Instruction` = '$instruction',`Link` = '$linkentered', `Frequency` = '$frequency', `FrequencyString` = '$frequencystring', `HowLong` = '$howlong', `HowLongType` = '$howlongtype', `IsCritical` = '$iscritical', `ResponseRequired` = '$responserequired' ,`SpecificTime` = '$specifictime', `StartFlag` = '$startflag', `NoOfDaysAfterPlanStarts` = '$numberofdaysafterplan', `SpecificDate` = '$specific_date', `UpdatedBy` = '$logged_userid' where `PlanCode` = '$current_created_plancode' and `PrescriptionNo` = '$presc_edit_id' and `RowNo` = '$currentrowid'";
					$update_query_run = mysql_query($update_query);

				} else {
					//INSERT NEW rows
					$max_rowno++;
					$insert_medicine_details = "insert into MEDICATION_DETAILS (`PlanCode`,`PrescriptionNo`,`RowNo`,`MedicineName`,`When`,`Instruction`,`Link`, `Frequency`,`FrequencyString`,`HowLong`,`HowLongType`,`IsCritical`,`ResponseRequired`,`StartFlag`,`NoOfDaysAfterPlanStarts`,`SpecificDate`,`SpecificTime`,`CreatedDate`,`CreatedBy`) values ('$current_created_plancode', '$presc_edit_id', '$max_rowno', '$medicinename','$when','$instruction','$linkentered','$frequency','$frequencystring','$howlong','$howlongtype','$iscritical','$responserequired','$startflag','$numberofdaysafterplan','$specific_date','$specifictime', now(), '$logged_userid');";
	...
	foreach ($existingids as $delid) {
		if($delid != ""){
			if(!in_array($delid, $usedmedids)){
				$delete_medicine_details = mysql_query("delete from MEDICATION_DETAILS where `PlanCode` = '$current_created_plancode' and `PrescriptionNo` = '$presc_edit_id' and `RowNo` = '$delid'");
			}
		}
	}
	header("location:plan_medication.php");
...
<html lang="en" class="no-js">
    ...
    <body id="wrapper">
    	...
    	<div class="navbar navbar-default" role="navigation" style="height: 100%;background-color:#004F35;position:fixed;width:16.7%;">
    		<div id="listBar">
		    	<div id="listHeader">
		    		<h6 style="font-family:Freestyle;font-size:180%;margin-top:-7px;letter-spacing:1px;">Prescription List</h6>
		    	</div>
		    	<div id="listItems">
		    	<button type="button" id="addItemButton" class="btns" style="display:none;">Add A Prescription<img src="images/addItem.png"></button>
		    		<?php 
		    		$get_plan_prescriptions 		= "select PrescriptionNo, PrescriptionName, DoctorsName, CreatedDate from MEDICATION_HEADER where PlanCode = '$current_created_plancode'";
    	...
    	<div class="col-lg-10 col-md-9 col-sm-9 col-sm-12 maincontent">
	    	<div id="dynamicPagePlusActionBar">
	    		<form name="frm_edit_prescription" id="frm_edit_prescription" method="post" action="">
		    	<?php 
		    		if(isset($presc_edit_id)){
		    			$get_medheader_to_edit = mysql_query("select PrescriptionName, DoctorsName from MEDICATION_HEADER where PrescriptionNo = '$presc_edit_id' and PlanCode = '$current_created_plancode'");
		    	...
		    	?>
		    	      <div class="prescriptionNameBar">
				        ...
				        <input type="text" name="doctorName" id="doctorName" placeholder="Enter Doctor Name.." maxlength="25" title="Enter the name of the doctor here.." value="<?php echo $doctor_name;?>">
					      ...
					      <tr id="pheader1">
					          <th style="max-width:250px;background-color:#C9C9C9;">Medicine Name</th>
				   ...
				   <?php 
				   		...
				   		$med_count = 0;
				   		$med_count_string = "";
				   		$get_meddetails_to_edit = mysql_query("select `RowNo`, `MedicineName`, `When`, `Instruction`,`Link`, `Frequency`, `FrequencyString`, `Howlong`, `HowLongType`, `IsCritical`, `ResponseRequired`, `StartFlag`, `NoOfDaysAfterPlanStarts`, `SpecificDate`, `SpecificTime` from MEDICATION_DETAILS where PrescriptionNo = '$presc_edit_id' and PlanCode = '$current_created_plancode'");
				   		...
				   		?>
		<tr>
          <td><textarea rows="2" name="medicine<?php echo $med_count;?>" id="medicine<?php echo $med_count;?>" placeholder="Enter Medicine Name.." maxlength="100" title="Enter Medicine Name here"><?php echo $medicinename;?></textarea></td>
          <td>
            <select name="when<?php echo $med_count;?>" id="when<?php echo $med_count;?>" title="Select the medicine dosage" class='whenshorthand'>
          ...
          <td>
            <select name="instruction<?php echo $med_count;?>" id="instruction<?php echo $med_count;?>" title="Select the medicine instruction"  <?php if($when == "16"){echo "disabled";}?>>
              <option value="0" style="display:none;">select</option>
          ...
          <td>
            <select name="frequency<?php echo $med_count;?>" id="frequency<?php echo $med_count;?>" title="Select the medicine frequency" class="medfrequency">
              <option value="0" style="display:none;">select</option>
