...
$request_types      = array("activities","settings_backup_restore","delete_plan","save_analytics","patient_contacts",
    "file_upload","reports","add_edit_reports","update_display_filename");
...
if($_REQUEST['RequestType']=="signup" && $_REQUEST['email_id']!="" && $_REQUEST['mobile_no']!="" && $_REQUEST['country_code']!="")
{
    ...
    if($duplicate_count==0)
    {
        ...
        if($email_id != "" && $first_name!="" && $check_insert)
        {
            ...
            $request            = "?userid=$userid&plancodeselected=$demo_plan_code";
            $hit_url            = $url.$request;
            //echo $hit_url;exit;
...
$user_count     = mysql_num_rows($validate_query);
    ...
    $user_info      = array();   
       ...
       // echo $split_country_code;exit;
        ...
        $p['LanguageID']            = (empty($user_row['LanguageID']))              ? '' : $user_row['LanguageID'];
        ...
        if($paid=='Y')
        {
           ...
           //echo $get_directory;exit;
            ...
            /*GET SELF PLANS*/
            $get_self_plans     =   "select ID,PlanName,PlanStartDate,PlanEndDate,Status 
            ...
            if($get_self_plan_count>0)
            {
                ...
                {
                    $self_plan_header_id            = $self_plan_rows['ID'];
                    $res1['SelfPlanHeaderID']       = $self_plan_header_id;
                    $res1['PlanName']               = $self_plan_rows['PlanName'];
                    $res1['PlanStartDate']          = date('Y-m-d',strtotime($self_plan_rows['PlanStartDate']));
                    $res1['PlanEndDate']            = date('Y-m-d',strtotime($self_plan_rows['PlanEndDate']));
                    $res1['PlanStatus']             = $self_plan_rows['Status'];

                    if($self_plan_header_id)
                    {
                        $get_activities =   "select ID,Type,Title,StartDate,FromTime,ToTime,Frequency,EndDate,Description,RemindBefore,Status
                                            from USER_SELF_PLAN_ACTIVITIES
                                            where UserSelfPlanHeaderID='$self_plan_header_id' and Status='A'";
                        $get_act_qry    = mysql_query($get_activities);
                        $get_act_count  = mysql_num_rows($get_act_qry);
                        if($get_act_count>0)
                        {
                        $activities         = array();
                            while($activity_rows   = mysql_fetch_array($get_act_qry))
                            {
                                $res2['ID']             = $activity_rows['ID'];
                                $res2['Type']           = $activity_rows['Type'];
                                $res2['Title']          = $activity_rows['Title'];
                                $res2['StartDate']      = $activity_rows['StartDate'];
                                $res2['FromTime']       = $activity_rows['FromTime'];
                                $res2['ToTime']         = $activity_rows['ToTime'];
                                $res2['Frequency']      = $activity_rows['Frequency'];
                                $res2['EndDate']        = $activity_rows['EndDate'];
                                $res2['Description']    = $activity_rows['Description'];
                                $res2['RemindBefore']   = $activity_rows['RemindBefore'];
                                $res2['Status']         = $activity_rows['Status'];
        ...
        if($get_reports_count>0)
        {
            while($analytics_row = mysql_fetch_array($get_reports_qry))
            {
                ...
                $rep['ReportID']        = $id;
                $rep['ReportName']      = (empty($analytics_row['ReportName']))     ? '' : $analytics_row['ReportName'];
                $rep['ReportGivenBy']   = (empty($analytics_row['ReportGivenBy']))  ? '' : $analytics_row['ReportGivenBy'];
               ...
               //echo $get_medication;exit;
                ...
                //END OF GET MEDICATION DETAILS
                 ...
                 //GET APPOINTMENT DETAILS
                    $get_appointment= "select distinct t2.AppointmentDate,t2.AppointmentTime,t2.AppointmentShortName,t2.DoctorsName,
...
*/
                ...
                $get_instruction = "select distinct t1.PrescriptionNo,t1.PrescriptionName,t1.DoctorsName,t2.MedicineName,t2.RowNo,t2.SpecificTime,t3.ShortHand,t2.Instruction,
                                    ...
                                    where t1.PlanCode=t2.PlanCode and t1.PrescriptionNo=t2.PrescriptionNo and t1.PlanCode='$plancode' 
                                    and t2.When=t3.ID and t1.UserID = t2.UserID and t1.UserID = '$userid' $timestamp_query";
                //echo $get_medication;exit;
...
elseif($_REQUEST['RequestType']=="activities" && $_REQUEST['plan_code']!="" && $_REQUEST['userid']!="")
{
    ...
    {
        $timestamp_query    = " and (t1.UpdatedDate>'$timestamp' or t2.UpdatedDate>'$timestamp') ";
        //$plan_timestamp     = " and t1.UpdatedDate>'$timestamp' ";
        $plan_timestamp  = "";
    }
...
elseif($_REQUEST['RequestType']=="settings_backup" && $_REQUEST['userid']!="")
{
    ...
    if($userid)
    {
        ...
        if($check_count)
        {//echo 123;exit;
            $settings_info  = "update USER_PHONE_SETTINGS set WakeUp='$wake_up',Morning='$morning',BeforeBreakfast='$before_breakfast',
                                ...
                                NormalVolume='$normal_volume',CriticalVolume='$critical_volume',NormalMusicFileName='$normal_music_file',
...
*/
                ...
                $get_instruction = "select distinct t1.PrescriptionNo,t1.PrescriptionName,t1.DoctorsName,t2.MedicineName,t2.RowNo,t2.SpecificTime,t3.ShortHand,t2.Instruction,
                                    ...
                                    from USER_INSTRUCTION_HEADER as t1,USER_INSTRUCTION_DETAILS as t2,MASTER_DOCTOR_SHORTHAND as t3
                                    where t1.PlanCode=t2.PlanCode and t1.PrescriptionNo=t2.PrescriptionNo and t1.PlanCode='$plancode' 
                                    and t2.When=t3.ID and t1.UserID = t2.UserID and t1.UserID = '$userid' ";
                //echo $get_medication;exit;
                $get_instruction_qry     = mysql_query($get_instruction);
                $get_instruction_count   = mysql_num_rows($get_instruction_qry);
                if($get_instruction_count)
    ...
    foreach($json as $each_value)
    {
        ...
        foreach ($each_value as $num)
        {
            ...
            foreach ($num as $key => $val)
            {
                ...
                $end_date       = date('Y-m-d', strtotime($start_date. " + $difference days"));
...
elseif($_REQUEST['RequestType']=="contact_support" && $_REQUEST['userid']!="" && $_REQUEST['plan_code']!=""  && $_REQUEST['callerid']!="" )
{
...
$support_mobile         = (empty($_REQUEST['callerid']))                ? '' : $_REQUEST['callerid'];
$status                 = 'IVR';
$random_id              = mt_rand(10,1000);
$uuid                   = $user_id.":".$section_id.":".$plancode.":".$prescno_or_selftestid.":".$row_no.":".$date.":".$time.":".$random_id;
...
elseif($_REQUEST['RequestType']=="reports" && $_REQUEST['user_id']!="" )
{
    ...
    if($get_reports_count>0)
    {
        while($analytics_row = mysql_fetch_array($get_reports_qry))
        {
            ...
            $res['ReportCreatedOn'] = (empty($analytics_row['ReportCreatedOn']))? '' : $analytics_row['ReportCreatedOn'];
...
elseif($_REQUEST['RequestType']=="self_plans" && $_REQUEST['user_id']!="")
{
    $userid             = (empty($_REQUEST['user_id']))             ? '' : $_REQUEST['user_id'];
    $self_plans         = array();
    $get_self_plans     =   "select ID,PlanName,PlanStartDate,PlanEndDate,Status 
...
//echo "<pre>";print_r($obj);exit;
    ...
    foreach($obj as $plan)
    {
        $check_duplicate        = mysql_query("select ID from USER_SELF_PLAN_HEADER where ID='".$plan['SelfPlanHeaderID']."'");
        $check_duplicate_count  = mysql_num_rows($check_duplicate);
        //echo $check_duplicate;exit;
        if($check_duplicate_count==1)
        {
            $plan_query = "update USER_SELF_PLAN_HEADER set PlanName='".$plan['PlanName']."',
                            PlanStartDate='".$plan['PlanStartDate']."',PlanEndDate='".$plan['PlanEndDate']."',
                            Status='".$plan['PlanStatus']."',UpdatedDate=now(),UpdatedBy='$userid'
                            where ID='".$plan['SelfPlanHeaderID']."'";

            foreach($plan['Activities'] as $act)
            {
                //echo "<pre>";print_r($abc);
                //echo $item['SelfPlanHeaderID'];exit;
                $check_duplicate_act        = mysql_query("select ID from USER_SELF_PLAN_ACTIVITIES 
                                                where ID='".$act['SelfPlanActivityID']."'");
                $check_duplicate_act_count  = mysql_num_rows($check_duplicate_act);
                //echo $check_duplicate;exit;
                if($check_duplicate_act_count==1)
                {
                $act_query = "update USER_SELF_PLAN_ACTIVITIES set Title='".$act['Title']."',Type='".$act['Type']."',
                            StartDate='".$act['StartDate']."',FromTime='".$act['FromTime']."',ToTime='".$act['ToTime']."',
                            Frequency='".$act['Frequency']."',EndDate='".$act['EndDate']."',Description='".$act['Description']."',
                            RemindBefore='".$act['RemindBefore']."',Status='".$act['Status']."',
                            UpdatedDate=now(),UpdatedBy='$userid'
                            where ID='".$act['SelfPlanActivityID']."' and UserSelfPlanHeaderID='".$plan['SelfPlanHeaderID']."'";
                }
                else
                {
                    //echo $item['Activities']['ActivityID'];
                $act_query = "insert into USER_SELF_PLAN_ACTIVITIES 
                            (ID,UserSelfPlanHeaderID,Title,Type,StartDate,FromTime,ToTime,Frequency,EndDate,Description,RemindBefore,Status,CreatedDate,CreatedBy) values 
                            ('".$act['SelfPlanActivityID']."','".$plan['SelfPlanHeaderID']."','".$act['Title']."','".$act['Type']."','".$act['StartDate']."','".$act['FromTime']."','".$act['ToTime']."','".$act['Frequency']."','".$act['EndDate']."','".$act['Description']."','".$act['RemindBefore']."','".$act['Status']."',now(),'$userid')";
                }
        ...
        {       
            $plan_query = "insert into USER_SELF_PLAN_HEADER 
                        (ID,UserID,PlanName,PlanStartDate,PlanEndDate,Status,CreatedDate,CreatedBy) values 
                        ('".$plan['SelfPlanHeaderID']."','$userid','".$plan['PlanName']."','".$plan['PlanStartDate']."','".$plan['PlanEndDate']."','".$plan['PlanStatus']."',now(),'$userid')";
...
elseif($_REQUEST['RequestType']=="notification_test"){
$message = "Mr X has not taken the medicine";
$userid  = "00091327942967327157";
$merchantid = "123456789";
session_start();
include('include/session.php');
if($merchantid == $logged_merchantid){
?>
