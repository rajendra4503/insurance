    ...
    <div class="col-sm-2 paddingrl0"  style="display:none;" id="sidebargrid">
        ...
        <button type="button" id="daysButton" class="btns">OK</button>
	    	...
	    	<div id="dynamicPagePlusActionBar">
	    		<label>
	    			You must first add a diet plan.  <span id='getdiet'>Click here</span> to start adding or Select A Template to get started.
	    		</label>
	    	</div>
    	</div>
    </div>
		...
		$(document).ready(function() {
			...
			var dietcount = 0;
			$('#plapiper_pagename').html("Diet");
			$('#addItemButton, #getdiet').click(function(){
				$.ajax({
					type        : "GET",
					url			: "dietDefaultPage.php",
					dataType	: "html",
