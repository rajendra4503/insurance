...
if((isset($_REQUEST['hidden_value']))&&(!empty($_REQUEST['hidden_value']))){
	...
	$self_edit_id = "1";
	if($selftestcount > '0'){
		$delete_user_header = mysql_query("delete from SELF_TEST_HEADER where  PlanCode = '$plancode_for_current_plan' and SelfTestID='$self_edit_id'");
		$delete_user_details = mysql_query("delete from SELF_TEST_DETAILS where  PlanCode = '$plancode_for_current_plan' and SelfTestID='$self_edit_id'");	
	}
	$insert_header_details 		= "insert into SELF_TEST_HEADER (Plancode, SelfTestID, CreatedDate, CreatedBy) values ('$plancode_for_current_plan', '$current_selftest_num', now(), '$logged_userid')";
    ...
    <!--END OF PLAN DETAILS MODAL WINDOW-->
        ...
        $('#topbar-leftmenu').click(function(){
	      if(sidebarflag == 1){
			...
			//alert(prescno);
			var dataString = "plancode="+plancode+"&type=get_master_selftests&testid="+selftestid+"&rowno="+rowno+"&userid="+userid;
				//bootbox.alert(dataString);
			$.ajax({
                  type    :"GET",
                  url     :"ajax_validation.php",
                    ...
                    $.each(data, function(i,item){
                      selftestcount = $('#selftestcount').val();
				        ...
				        propercount     = parseInt(propercount) + 1;
				         $('.deleterow').show();
				        var first = "<table id='pdata'><tr><td><select name='selftestName"+propercount+"' id='selftestName"+propercount+"'><option style='display:none;' value='0'>select</option>"+item.MedicalTestOptions+"</select></td><td><input type='text' name='doctorName"+propercount+"' id='doctorName"+propercount+"' placeholder='Enter Doctor Name..' class='forminputs2' style='max-width:250px;' maxlength='25' value='"+item.DoctorsName+"'></td><td><select name='when"+propercount+"' id='when"+propercount+"'><option style='display:none;' value='0'>select</option>"+item.InstructionOptions+"</select></td><td><select name='frequency"+propercount+"' id='frequency"+propercount+"' class='testfrequency'>'"+item.FrequencyOptions+"'</select></td><td><div style='border-radius:10px;padding:5px;background-color:#004f35;height:40px;width:150px;'><input type='text' maxlength='2' name='count"+propercount+"' id='count"+propercount+"' style='width:35px;height:30px;' class='forminputs2 roundedinputs countbox' title='Enter the duration' value='"+item.HowLong+"'><select class='' id='countType"+propercount+"' name='countType"+propercount+"' style='width:100px;float:right;height:30px;line-height:25px;background-color:#2B6D57;' title='Enter the duration'><option value='0' style='display:none;'>select</option>"+item.HowLongTypeOptions+"</select></div></td><td><input type='checkbox' name='response"+propercount+"' id='response"+propercount+"' checked style='display:none;'></td><td><img src='images/closeRow.png' width='30px' height='auto' class='deleterow' id='"+propercount+"'></td></tr><tr><td colspan='3' style='padding:0px'><textarea name='selftestdesc"+propercount+"' placeholder='Enter Requirements to selftest..' class='forminputs2' id='selftestdesc"+propercount+"'>"+item.TestDescription+"</textarea></td><td colspan='4' style='padding:5px;'> <input "+item.WeeklyType+" name='selectedweekdays"+propercount+"' id='selectedweekdays"+propercount+"' value='"+item.FrequencyString+"' class='forminputs2 editselectedweekday' readonly title='Click here to edit frequency'><input "+item.MonthlyType+" name='selectedmonthdays"+propercount+"' id='selectedmonthdays"+propercount+"' value='"+item.FrequencyString+"' class='forminputs2 editselectedmonthday' readonly title='Click here to edit frequency'></td></tr><tr><td colspan='6'><input type='text' name='linkentered"+propercount+"' id='linkentered"+propercount+"' class='forminputs2' title='Enter Link here' placeholder='Enter Link Here (Optional)' style='width:100%;' value='"+item.Link+"'></td></tr><tr style='border-bottom:4px solid #000;'><td colspan='7' style='padding:0px;text-align:left'><label>Start:</label>&nbsp;&nbsp;&nbsp;<input type='radio' name='radio"+propercount+"' value='PS' checked class='radio"+propercount+" selfriptionradio' "+item.PlanStartRadio+"> When the plan Starts/Updates<input type='radio' name='radio"+propercount+"' value='ND' class='radio"+propercount+" selfriptionradio' "+item.NumOfDaysRadio+"> No. Of days after plan started&nbsp;<input type='text' size='6px' name='numofdays"+propercount+"' id='numofdays"+propercount+"' class='numofdays  maxlength='2' value='"+item.NoOfDaysAfterPlanStarts+"'><input type='radio' name='radio"+propercount+"' value='SD' class='radio"+propercount+" selfriptionradio' "+item.SpecificDateRadio+"> At Specific Date:&nbsp;<input type='text' size='10px' name='specificdate"+propercount+"' id='specificdate"+propercount+"' class='specificdate' value='"+item.SpecificDate+"'></td></tr></table>";
