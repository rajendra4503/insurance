...
if((isset($_REQUEST['prescriptionName']))&&(!empty($_REQUEST['prescriptionName']))){
	...
	$delete_presc_ 	= mysql_query("delete from INSTRUCTION_HEADER where PlanCode = '$plancode_for_current_plan'");
	$delete_all_prescriptions 	= mysql_query("delete from INSTRUCTION_DETAILS where PlanCode = '$plancode_for_current_plan'");
	$get_last_prescription_num 	= mysql_query("select max(PrescriptionNo) from INSTRUCTION_HEADER where PlanCode = '$plancode_for_current_plan'");
	...
	$insert_header_details 	= " insert into INSTRUCTION_HEADER (Plancode, PrescriptionNo, PrescriptionName, DoctorsName, CreatedDate, CreatedBy) values ('$plancode_for_current_plan', '$current_presc_num', '$prescription_name', '$doctor_name', now(), '$logged_userid')";
	...
	foreach ($medicineids as $ids) {
		if($ids != ""){
			...
			if($medicinename != ""){
			$instructiontypes = mysql_real_escape_string(trim(htmlspecialchars($_REQUEST["instructiontypes$ids"])));
		...
		$insert_medicine_details = "insert into INSTRUCTION_DETAILS (`PlanCode`,`PrescriptionNo`,`RowNo`,`MedicineName`, `InstructionTypeID`, `ThresholdLimit`,`When`,`SpecificTime`,`Instruction`,`Link`, `OriginalFileName`, `Frequency`,`FrequencyString`,`HowLong`,`HowLongType`,`IsCritical`,`ResponseRequired`,`StartFlag`,`NoOfDaysAfterPlanStarts`,`SpecificDate`,`CreatedDate`,`CreatedBy`) values ('$plancode_for_current_plan', '$current_presc_num', '$count', '$medicinename', '$instructiontypes', '$threshold','$when','$specifictime','$instruction','$fullpath','$filename','$frequency','$frequencystring','$howlong','$howlongtype','$iscritical','$responserequired','$startflag','$numberofdaysafterplan','$specific_date', now(), '$logged_userid')";
		//$insert_medicine_details = "insert into INSTRUCTION_DETAILS (`PlanCode`,`PrescriptionNo`,`RowNo`,`MedicineName`,`When`,`SpecificTime`,`Instruction`,`Link`, `Frequency`,`FrequencyString`,`HowLong`,`HowLongType`,`IsCritical`,`ResponseRequired`,`StartFlag`,`NoOfDaysAfterPlanStarts`,`SpecificDate`,`CreatedDate`,`CreatedBy`) values ('$plancode_for_current_plan', '$current_presc_num', '$count', '$medicinename','$when','$specifictime','$instruction','$linkentered','$frequency','$frequencystring','$howlong','$howlongtype','$iscritical','$responserequired','$startflag','$numberofdaysafterplan','$specific_date', now(), '$logged_userid')";
	...
	//$specifictime = date('H:i:s',strtotime($specifictime));
	$insert_medicine_details = "insert into INSTRUCTION_DETAILS (`PlanCode`, `PrescriptionNo`, `RowNo`, `MedicineName`, `InstructionTypeID`, `ThresholdLimit`, `When`, `Instruction`, `Link`, `OriginalFileName`, `Frequency`,`FrequencyString`,`HowLong`,`HowLongType`,`IsCritical`,`ResponseRequired`,`StartFlag`,`NoOfDaysAfterPlanStarts`,`SpecificDate`,`CreatedDate`,`CreatedBy`) values ('$plancode_for_current_plan', '$current_presc_num', '$count', '$medicinename', '$instructiontypes', '$threshold','$when','$instruction','$fullpath','$filename','$frequency','$frequencystring','$howlong','$howlongtype','$iscritical','$responserequired','$startflag','$numberofdaysafterplan','$specific_date', now(), '$logged_userid')";
	//$insert_medicine_details = "insert into INSTRUCTION_DETAILS (`PlanCode`,`PrescriptionNo`,`RowNo`,`MedicineName`,`When`,`Instruction`,`Link`, `Frequency`,`FrequencyString`,`HowLong`,`HowLongType`,`IsCritical`,`ResponseRequired`,`StartFlag`,`NoOfDaysAfterPlanStarts`,`SpecificDate`,`CreatedDate`,`CreatedBy`) values ('$plancode_for_current_plan', '$current_presc_num', '$count', '$medicinename','$when','$instruction','$linkentered','$frequency','$frequencystring','$howlong','$howlongtype','$iscritical','$responserequired','$startflag','$numberofdaysafterplan','$specific_date', now(), '$logged_userid')";
		...
		$check_affected_rows= mysql_affected_rows();
		       ...
		       {
		       	header("Location:plan_inst_new.php");
		       } else {
		       	header("Location:plan_inst_new.php");
		       }
		}
...
<html lang="en" class="no-js">
    ...
    <div class="col-sm-2 paddingrl0"  style="display:none;" id="sidebargrid">
      ...
      <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 paddingrl0" align="center"  id="plantitle"><span id="thisplantitle" title="Click to edit the plan details">

     <?php if(isset($ClicktoeditPlanDetails) &&  $ClicktoeditPlanDetails !=''){echo $ClicktoeditPlanDetails;} else{echo $planname_for_current_plan_text;}?>
      		

      	</span><span title="Click to edit the plan details" id="editthisplantitle">&nbsp;&nbsp;<img src="images/editad.png" style="height:20px;cursor:pointer;"></span></div>
      <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 paddingrl0" align="right"  id="plantitle"><button type="button" class="btns" align="right" id="finished_adding"><img src="images/finishAdd.png" style="height:20px;width:auto;margin-bottom:3px;">&nbsp;	<?php if(isset($FINISHADDING) &&  $FINISHADDING !=''){echo $FINISHADDING;} else{echo 'FINISH ADDING';}?></button></div>
</div>
    	...
    	<div class="navbar navbar-default" role="navigation" style="height: 100%;background-color:#004F35;position:fixed;width:16.7%;">
    		...
    		<h6 style="font-family:Freestyle;font-size:33px;margin-top:-1px;letter-spacing:1px;color:#f2bd43;background-color:#000;"><?php if(isset($PrescriptionList) &&  $PrescriptionList !=''){echo $PrescriptionList;} else{echo 'Prescription List';}?></h6>
    		<div class="sidebarheadings"><?php if(isset($MasterPrescriptions) &&  $MasterPrescriptions !=''){echo $MasterPrescriptions;} else{echo 'Master Prescriptions';}?> :</div>
			    		...
			    		t1.DoctorsName, t1.CreatedDate from INSTRUCTION_HEADER as t1,PLAN_HEADER as t2 
		    		...
		    		if($get_plan_prescriptions_count > 0){
			    			 ...
			    			 <div class="panel panel-default plancreationpanel">
					              ...
					              <div id="collapse<?php echo $prescription_code.$prescription_no; ?>" class="panel-collapse collapse plancreationpanelbody" role="tabpanel" aria-labelledby="heading<?php echo $prescription_code.$prescription_no; ?>">
					                <div class="panel-body"  style="text-align:center;">
					                	<?php
					                		$med_details = mysql_query("select MedicineName from INSTRUCTION_DETAILS where PlanCode = '$prescription_code' and PrescriptionNo='$prescription_no'");
		    			...
		    			echo "<div style='color:#fff;'>No Instructions to show</div>";
		    		}
			    		?>
		    		</div>
		    		<div class="sidebarheadings" style="margin-top: -13px;">Assigned Instructions :</div>
			    		...
			    		$get_plan_prescriptions 		= "select t1.UserID,t1.PrescriptionNo,t1.PlanCode,
			    		 t1.PrescriptionName, t1.DoctorsName, t1.CreatedDate from USER_INSTRUCTION_HEADER as t1,USER_PLAN_HEADER as t2 
			    			...
			    			?>
			    			 <div class="panel panel-default plancreationpanel">
					              ...
					              <div id="collapse<?php echo $prescription_user.$prescription_code.$prescription_no; ?>" class="panel-collapse collapse plancreationpanelbody" role="tabpanel" aria-labelledby="heading<?php echo $prescription_user.$prescription_code.$prescription_no; ?>">
					                <div class="panel-body" style="text-align:center;">
					                	<?php
					                		$med_details = mysql_query("select MedicineName from USER_INSTRUCTION_DETAILS where PlanCode = '$prescription_code' and PrescriptionNo='$prescription_no' and UserID='$prescription_user'");
					                		$med_details_count = mysql_num_rows($med_details);
					                		if($med_details_count > 0){
						                		while ($med_row = mysql_fetch_array($med_details)) {
						                			$medicine_name = $med_row['MedicineName'];
    	...
    	<div class="col-lg-10 col-md-9 col-sm-9 col-sm-12 maincontent">
	    	<div id="dynamicPagePlusActionBar">
 			    <label>
	    			You must first add an Instruction to include all the Medicines. <span id='getmedications'>Click here</span> to start adding or Select A Template to get started.<br>
	    		</label>
			</div>
    	</div>
    </div>
		...
		$(document).ready(function() {
			...
			$('#addItemButton, #getmedications').click(function(){
				if (medicationcount > 0){
					var discard = confirm("The current instruction will be discarded. Click OK to continue.");
					if(discard == true){
						medicationcount = 0;
					} else {
				...
				if(medicationcount == 0){
					$.ajax({
						type        : "GET",
						url			: "instructionDefaultPage4.php",
						dataType	: "html",
						success	: function (response)
						{
...
$('.panel-heading a').on('click',function(e){
	    ...
	    $('.panel-collapse').removeClass('in');		
			...
			$(document).on('change', '.prescriptionradio', function () {
				...
				if(current_prescription_name.replace(/\s+/g, '') == ""){
					bootbox.alert("Please enter a name for this template.");
					$('.bootbox').on('hidden.bs.modal', function() {
				...
				for (i = 0; i < medicationcount; ++i) {
				 	...
				 	if(!medicinename == ""){
				 		...
				 		var instructiontypeinput = $('#instructiontypes'+result[i]).val();
				 		//bootbox.alert(wheninput);
				 		if(instructiontypeinput == 0){
				 			bootbox.alert("Please select the instruction type");
				 			$('.bootbox').on('hidden.bs.modal', function() {
				 				$('#instructiontypes'+result[i]).focus();
				 			});
				 			return false;
				 		}
	...
	//EDIT MEDICATION BUTTON CLICKED - FROM SIDE PANEL
		...
		$('.addassignedplanprescriptions').click(function(){
			...
			//alert(prescno);
			var dataString = "plancode="+plancode+"&type=get_assigned_instructions&prescno="+prescno+"&userid="+userid;
				//bootbox.alert(dataString);
				$.ajax({
                  type    :"GET",
                  url     :"ajax_validation.php",
		...
		$('.addmasterplanprescriptions').click(function(){
			...
			var dataString = "plancode="+plancode+"&type=get_master_instructions&prescno="+prescno;
				//alert(dataString);
				$.ajax({
                  type    :"GET",
                  url     :"ajax_validation.php",
