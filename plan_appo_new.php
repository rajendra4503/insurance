...
if((isset($_REQUEST['hidden_value']))&&(!empty($_REQUEST['hidden_value']))){
	...
	$delete_user_header = mysql_query("delete from APPOINTMENT_HEADER where  PlanCode = '$plancode_for_current_plan'");
	$delete_user_details = mysql_query("delete from APPOINTMENT_DETAILS where  PlanCode = '$plancode_for_current_plan'");	
	}
	...
	foreach ($appointmentids as $ids) {
		if($ids != ""){
			$appointment_name =  mysql_real_escape_string(trim(htmlspecialchars($_REQUEST["appointmentName$ids"])));
			if($appointment_name != ""){
			$doctor_name 				= mysql_real_escape_string(trim(htmlspecialchars($_REQUEST["doctorName$ids"])));
			$appointment_date =  mysql_real_escape_string(trim(htmlspecialchars($_REQUEST["appointment_date$ids"])));
			...
			$appointment_time =  mysql_real_escape_string(trim(htmlspecialchars($_REQUEST["appointment_time$ids"])));
			$appointment_time 		=  date('H:i:s',strtotime($appointment_time));
			$duration_inhours 		= $_REQUEST["duration_inhours$ids"];
			$duration_inmins  		= $_REQUEST["duration_inmins$ids"];
			$appointmentduration 	= $duration_inhours.":".$duration_inmins;
			$appointment_venue 		= $_REQUEST["venue$ids"];
			$appointment_reqs =  mysql_real_escape_string(trim(htmlspecialchars($_REQUEST["requirements$ids"])));
			$insert_appointment_details = "insert into APPOINTMENT_DETAILS (`PlanCode`, `AppointmentDate`, `AppointmentTime`, `AppointmentDuration`, `AppointmentPlace`, `AppointmentShortName`, `DoctorsName`, `AppointmentRequirements`, `CreatedDate`, `CreatedBy`) values ('$plancode_for_current_plan', '$appointment_date', '$appointment_time','$appointmentduration', '$appointment_venue', '$appointment_name','$doctor_name','$appointment_reqs', now(), '$logged_userid');";
	...
	foreach ($datearray as $date) {
		...
		header("Location:plan_appo_new.php");
}
    ...
    <div class="col-sm-2 paddingrl0"  style="display:none;" id="sidebargrid">
      ...
      <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 paddingrl0" align="right"  id="plantitle"><button type="button" class="btns" align="right" id="finished_adding"><img src="images/finishAdd.png" style="height:20px;width:auto;margin-bottom:3px;">&nbsp;<?php if(isset($FINISHADDING) &&  $FINISHADDING !=''){echo $FINISHADDING;} else{echo 'FINISH ADDING';}?></button></div>
    	...
    	<div class="navbar navbar-default" role="navigation" style="height: 100%;background-color:#004F35;position:fixed;width:16.7%;">
    		...
    		<h6 style="font-family:Freestyle;font-size:33px;margin-top:-1px;letter-spacing:1px;color:#f2bd43;background-color:#000;"><?php if(isset($AppointmentList ) &&  $AppointmentList !=''){echo $AppointmentList;} else{echo 'Appointment List ';}?></h6>
    		<div class="sidebarheadings"><?php if(isset($MasterAppointments) &&  $MasterAppointments !=''){echo $MasterAppointments;} else{echo 'Master Appointments ';}?>:</div>
    			<div class="panel-group masterplanactivities" id="accordion1" role="tablist" aria-multiselectable="true" style="max-height:250px;overflow:scroll;overflow-x:hidden;">
		    		...
		    		<div class="sidebarheadings" style="margin-top: -13px;"><?php if(isset($AssignedAppointments) &&  $AssignedAppointments !=''){echo $AssignedAppointments;} else{echo 'Assigned Appointments ';}?>:</div>
		...
		$(document).ready(function() {
			...
			$('#addItemButton, #getappointments').click(function(){
				...
				if(appointmentcount == 0){
					$.ajax({
						type        : "GET",
						url			: "appointmentsDefaultPage4.php",
						dataType	: "html",
						success	: function (response)
						{ 
