...
if((isset($_REQUEST['hidden_value']))&&(!empty($_REQUEST['hidden_value']))){
	...
	if($labtestcount > '0'){
		$delete_user_header = mysql_query("delete from USER_LAB_TEST_HEADER1 where  PlanCode = '$plan_to_customize' and UserID='$assigned_to_user' and LabTestID='$lab_edit_id'");
		$delete_user_details = mysql_query("delete from USER_LAB_TEST_DETAILS1 where  PlanCode = '$plan_to_customize' and UserID='$assigned_to_user' and LabTestID='$lab_edit_id'");	
	}
	$insert_header_details 	= " insert into USER_LAB_TEST_HEADER1 (UserID, Plancode, LabTestID, CreatedDate, CreatedBy) values ('$assigned_to_user','$plan_to_customize', '$current_labtest_num', now(), '$logged_userid')";
	...
	foreach ($labttestids as $ids) {
		if($ids != ""){
			...
			$insert_labttest_details = "insert into USER_LAB_TEST_DETAILS1 (`UserID`,`PlanCode`,`LabTestID`,`RowNo`,`TestName`,`DoctorsName`,`LabTestRequirements`,`CreatedDate`,`CreatedBy`) values ('$assigned_to_user','$plan_to_customize', '$current_labtest_num', '$count', '$labtestName','$doctor_name','$requirements', now(), '$logged_userid');";
			...
			$check_affected_rows= mysql_affected_rows();
		       ...
		       { 
		       	header("Location:customize_labtest.php");
		       } else {
		       	header("Location:customize_labtest.php");
		       }
		}
...
<html lang="en" class="no-js">
    <head>
		...
		<title>Plan Piper | Customize Lab Tests</title>
...
<button type="button" class="btns" align="right" id="finished_adding"><img src="images/finishAdd.png" style="height:20px;width:auto;margin-bottom:3px;">&nbsp;FINISH CUSTOMIZING</button></div>
			...
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				<ul class="nav nav-pills nav-justified">
					...
					<li role="presentation" class="active navbartoptabs"><a href="customize_labtest.php">LAB TEST</a></li>
    	...
    	<div class="navbar navbar-default" role="navigation" style="height: 100%;background-color:#004F35;position:fixed;width:16.7%;">
    		<div id="listBar">
		    	...
		    	<div id="listItems">
		    		...
		    		<?php 
		    		$get_plan_labtests 		= "select PlanCode, LabTestID, CreatedDate from USER_LAB_TEST_HEADER1 where PlanCode = '$plan_to_customize' and UserID='$assigned_to_user'";
    	...
    	<div class="col-lg-10 col-md-9 col-sm-9 col-sm-12 maincontent">
	    	<div id="dynamicPagePlusActionBar">
	    		   <form name="frm_customize_labtest" id="frm_customize_labtest" method="post" action="customize_labtest.php?id=<?php echo $lab_edit_id;?>">
			      ...
			      <table id="adata" class="table table-striped">
			        ...
			        <?php if(isset($lab_edit_id)){
			        	...
			        	$getlabtests = mysql_query("select TestName, DoctorsName, LabTestRequirements from USER_LAB_TEST_DETAILS1 where LabTestID='$lab_edit_id' and PlanCode='$plan_to_customize' and UserID='$assigned_to_user'");
