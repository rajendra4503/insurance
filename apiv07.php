...
$res1['Link']  = (empty($medication_rows['Link'])) ? '' : $host_server.$medication_rows['Link'];
                   ...
                   //echo $get_appointment.'<br>';exit;
                    ...
                    if($get_self_test_count)
                    {
                        while($self_test_rows=mysql_fetch_array($get_self_test_qry))
                        {
                            ...
                            $res31['Link'] = (empty($self_test_rows['Link']))                      ? '' : $host_server.$self_test_rows['Link'];
                        ...
                        array_push($result,$res31);
                             if($MedicalTestID == "5"){
                                ...
                                $res8['Link']  = (empty($self_test_rows['Link']))                     ? '' : $host_server.$self_test_rows['Link'];
                    ...
                    if($get_diet_detials_count)
                    {
                        while($diet_rows=mysql_fetch_array($get_diet_detials_qry))
                        {
                            ...
                            $res4['Link']                       = (empty($diet_rows['Link']))               ? '' : $host_server.$diet_rows['Link'];
...
*/
                ...
                if($get_instruction_count)
                {
                    while($instruction_rows=mysql_fetch_array($get_instruction_qry))
                    {
                       ...
                       if($st != ""){
                        ...
                        $res8['Link']                   = (empty($instruction_rows['Link']))                     ? '' : $host_server.$instruction_rows['Link'];
                        $res8['AppointmentDate']        = "";   $res8['AppointmentTime']        = "";   $res8['AppointmentShortName']   = "";
                        $res8['AppointmentRequirements']= "";   $res8['LabTestDate']            = "";   $res8['LabTestTime']            = "";
                        $res8['TestName']               = "";   $res8['SelfTestID']             = "";   $res8['TestDescription']        = "";   
                        $res8['DietNo']                 = "";   $res8['DietPlanName']           = "";   $res8['AdvisorName']            = "";   
    ...
    echo "{".json_encode('PLANPIPER_LOGIN').':'.json_encode($loginstatus).","
            ...
            .json_encode('PLANS').':'.json_encode($plan_details,JSON_UNESCAPED_SLASHES).","
   ...
   // $user_id              = (empty($_REQUEST['user_id']))             ? '' : $_REQUEST['user_id'];
    ...
    if($get_notification_detail_count>0)
    {
        while($notification_detail_rows   = mysql_fetch_array($get_notification_details_qry))
        {
            ...
            $notification_details['Link']                   = (empty($notification_detail_rows['Link']))    ? '': $host_server.$notification_detail_rows['Link'];
...
elseif($_REQUEST['RequestType']=="language_response" && $_REQUEST['language'] !=""){
	...
	if( $language != '' ){
        ...
        // Mobile Login data
		$loginData = array();
		$query  = mysql_query("SELECT FieldNo ,$language  FROM $language WHERE Type = 'Mobile' AND ScreenNo='0001'");

		while ( $result = mysql_fetch_assoc($query) ) {
		$loginData[$result['FieldNo']] = $result[$language];
		}

        // Mobile Registration data 
		$registerData = array();
		$query  = mysql_query("SELECT FieldNo ,$language  FROM $language WHERE Type = 'Mobile' AND ScreenNo='0002'");

		while ( $result = mysql_fetch_assoc($query) ) {
		$registerData[$result['FieldNo']] = $result[$language];
		}

        // Mobile Menu data
		$menuData = array();
		$query  = mysql_query("SELECT FieldNo ,$language  FROM $language WHERE Type = 'Mobile' AND ScreenNo='0003'");

		while ( $result = mysql_fetch_assoc($query) ) {
		$menuData[$result['FieldNo']] = $result[$language];
		}


		// My plans Data
		$myplansData = array();
		$query  = mysql_query("SELECT FieldNo ,$language  FROM $language WHERE Type = 'Mobile' AND ScreenNo='0004'");

		while ( $result = mysql_fetch_assoc($query) ) {
		$myplansData[$result['FieldNo']] = $result[$language];
		}
		
	}

	echo "{"
	.json_encode('Login').':'.json_encode($loginData,JSON_UNESCAPED_UNICODE).","
	.json_encode('Registration').':'.json_encode($registerData,JSON_UNESCAPED_UNICODE).","
	.json_encode('Menu').':'.json_encode($menuData,JSON_UNESCAPED_UNICODE).","
	.json_encode('MyPlans').':'.json_encode($myplansData,JSON_UNESCAPED_UNICODE).
	"}";
    die;
}
?>