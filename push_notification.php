...
//echo "<pre>";print_r($_SESSION);exit;
	...
	if( !empty( $_REQUEST['lang']) && $_REQUEST['lang'] != '' ){
		...
		if(table_exists($table)){
		  $query  = mysql_query("SELECT FieldNo ,$table  FROM $table WHERE ScreenNo='006'");
...
<html lang="en" class="no-js">
	...
	<div id="planpiper_wrapper">
		 ...
		 <div class="col-sm-10 paddingrl0" id="content_wrapper">
		 	...
		 	<section>
		 		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 paddingrl0"><div id="plantitle">Send Notification <?php if(isset($SendNotification) &&  $SendNotification !=''){echo '('.$SendNotification.')';}?></div></div>
			 	...
			 	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 paddingrl0">
				 	...
				 	<div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
				 		<input type="checkbox" name='selectall' value='1' class='selectall' id='selectall'> Send To all <?php if(isset($SendToall) &&  $SendToall !=''){echo '('.$SendToall.')';}?>
				 	</div>			 		
			 	</div>
			 	<input type="text" class="forminputs3 firstlettercaps" placeholder="Title <?php if(isset($Title) &&  $Title !=''){echo '('.$Title.')';}?>" name="title" id="title" maxlength="30" style='height:35px;border:1px solid #004f35;margin-top:10px;'>
			 	<textarea class="forminputstextarea" id="description" name="description" rows="3" maxlength='100' style="margin-top:10px;border:1px solid #004f35;margin-bottom:4px;" placeholder="Enter Notification Content here <?php if(isset($EnterNotification) &&  $EnterNotification !=''){echo '('.$EnterNotification.')';}?>"></textarea>
		 		...
		 		<div class="fileinput fileinput-new input-group col-xs-12" data-provides="fileinput" style="width:100%;height:35px;border:1px solid #004f35;margin-top:5px;">
                      ...
                      <span class="input-group-addon btn btn-default btn-file"><span class="fileinput-new">Click To Upload A Document (Optional) <?php if(isset($ClickUpload) &&  $ClickUpload !=''){echo '('.$ClickUpload.')';}?></span><span class="fileinput-exists">Change</span><input type="file"  name="uploadedfile" id="uploadedfile"></span>
                      <a href="#" class="input-group-addon btn btn-default fileinput-exists removelinkbutton" data-dismiss="fileinput">Remove</a>
                    </div>
                <input type="text" class="forminputs3" placeholder="Enter a Link (Optional)  <?php if(isset($EnterLink) &&  $EnterLink !=''){echo '('.$EnterLink.')';}?>" name="link" id="link" maxlength="250" style='height:35px;border:1px solid #004f35;'>
		 		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" align="center" style="margin-top:25px;">
					...
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" id="ActionBar2" align="center" style="margin-top:25px;">
						        <button id="sendnotificationbutton" class="formbuttonsmall">SEND <?php if(isset($SEND) &&  $SEND !=''){echo '('.$SEND.')';}?></button>
						        </div>
				</form>		      
				</div>
		 	</section>
