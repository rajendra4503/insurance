...
include('include/functions.php');
  ...
  if( !empty( $_REQUEST['lang']) && $_REQUEST['lang'] != '' ){
    ...
    if(table_exists($table)){
      $query  = mysql_query("SELECT FieldNo ,$table  FROM $table WHERE ScreenNo='004'");
...
<html lang="en" class="no-js">
	...
	<div id="planpiper_wrapper">
      ...
      ?>
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 paddingrl0"><div id="plantitle"><span style="padding-left:165px;"><?php echo $title;?>  <?php if(isset($PatientList) &&  $PatientList !=''){echo '('.$PatientList.')';}?>  </span><a href="new_user.php" style="color:#004f35;text-decoration:none;"><button type="button" class="btns" align="right" id="finished_adding"><?php echo $button;?> <?php if(isset($AddNewPatient) &&  $AddNewPatient !=''){echo '('.$AddNewPatient.')';}?></button></a></div></div>         
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 paddingrl0" style="margin-top:5px;" id="mainplanlistdiv">
          ...
          <div class="table-responsive">
              ...
              <tr class="tableheadings">
                  <th>#</th>
                  <th>Name <br><?php if(isset($Name) &&  $Name !=''){echo '('.$Name.')';}?></th>
                  <th>Email<br><?php if(isset($Email) &&  $Email !=''){echo '('.$Email.')';}?></th>
                  <th>Mobile<br><?php if(isset($Mobile) &&  $Mobile !=''){echo '('.$Mobile.')';}?></th>
                  <th>Number Of Plans <br><?php if(isset($NumberOfPlans) &&  $NumberOfPlans !=''){echo '('.$NumberOfPlans.')';}?></th>
                  <th>Review<br><?php if(isset($Review) &&  $Review !=''){echo '('.$Review.')';}?></th>
                  <th>Edit<br><?php if(isset($Edit) &&  $Edit !=''){echo '('.$Edit.')';}?></th>
                  <th>Deactivate<br><?php if(isset($Deactivate) &&  $Deactivate !=''){echo '('.$Deactivate.')';}?></th>
              </tr>
            </thead>
              <tbody>
          <?php 
          ...
          if($admin_count > 0){
            while ($admins = mysql_fetch_array($get_admins)) {
              ...
              $get_plan_count = mysql_query("select count(*) from USER_PLAN_HEADER where UserID='$user_id' and MerchantID='$logged_merchantid'");
