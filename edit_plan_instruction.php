...
$get_max_rowno = mysql_query("select max(RowNo) from INSTRUCTION_DETAILS where `PlanCode` = '$current_created_plancode' and `PrescriptionNo` = '$presc_edit_id'");
...
if((isset($_REQUEST['prescriptionName']))&&(!empty($_REQUEST['prescriptionName']))){
	...
	$update_query 				= "update INSTRUCTION_HEADER set `PrescriptionName` = '$prescription_name', `DoctorsName` = '$doctor_name', `UpdatedBy` = '$logged_userid' WHERE `PlanCode` = '$current_created_plancode' AND `PrescriptionNo` = '$presc_edit_id'";
	...
	foreach ($usedmedids as $ids) {
		...
		if($delid != ""){
			if(!in_array($delid, $usedmedids)){
				$delete_medicine_details = mysql_query("delete from INSTRUCTION_DETAILS where `PlanCode` = '$current_created_plancode' and `PrescriptionNo` = '$presc_edit_id' and `RowNo` = '$delid'");
			}
		}
	}
    ...
    <div class="col-sm-2 paddingrl0"  style="display:none;" id="sidebargrid">
    	...
    	<div class="navbar navbar-default" role="navigation" style="height: 100%;background-color:#004F35;position:fixed;width:16.7%;">
    		<div id="listBar">
		    	...
		    	<div id="listItems">
		    	<button type="button" id="addItemButton" class="btns" style="display:none;">Add An Instruction<img src="images/addItem.png"></button>
		    		<?php 
		    		...
		    		if($get_plan_prescriptions_count > 0){
			    		while ($get_presc_row = mysql_fetch_array($get_plan_prescriptions_run)) {
			    			...
			    			?>
			    			<button type="button" class="btns editinstructionbuttons" id="<?php echo $prescription_no; ?>">
    	...
    	<div class="col-lg-10 col-md-9 col-sm-9 col-sm-12 maincontent">
	    	<div id="dynamicPagePlusActionBar">
	    		<form name="frm_edit_instruction" id="frm_edit_instruction" method="post" action="">
		    	<?php 
		    		if(isset($presc_edit_id)){
		    			$get_medheader_to_edit = mysql_query("select PrescriptionName, DoctorsName from INSTRUCTION_HEADER where PrescriptionNo = '$presc_edit_id' and PlanCode = '$current_created_plancode'");
		    	...
		    	?>
		    	      <div class="prescriptionNameBar">
				        ...
				        <input type="text" name="prescriptionName" id="prescriptionName" placeholder="Template Name.." maxlength="25" title="Enter a name for the template" value="<?php echo $prescription_name; ?>">
				        </div>
				   ...
				   if(isset($presc_edit_id)){
				   		$med_count = 0;
				   		$med_count_string = "";
				   		$get_meddetails_to_edit = mysql_query("select `RowNo`, `MedicineName`, `When`, `Instruction`,`Link`, `Frequency`, `FrequencyString`, `Howlong`, `HowLongType`, `IsCritical`, `ResponseRequired`, `StartFlag`, `NoOfDaysAfterPlanStarts`, `SpecificDate`, `SpecificTime` from INSTRUCTION_DETAILS where PrescriptionNo = '$presc_edit_id' and PlanCode = '$current_created_plancode'");
		  ...
		  $(document).on('click', '.deleterow', function () {
		   ...
		   if(medicine_name.replace(/\s+/g, '') == ""){
			...
			$(document).on('change', '.prescriptionradio', function () {
				...
				$('#frm_edit_instruction').submit();
			});
				//EDIT MEDICATION BUTTON CLICKED - FROM SIDE PANEL
		$('.editinstructionbuttons').click(function(){
