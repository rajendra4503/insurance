...
if((isset($_REQUEST['hidden_value']))&&(!empty($_REQUEST['hidden_value']))){
	...
	foreach ($selfttestids as $ids) {
		if($ids != ""){
			...
			//$deletelink = $_REQUEST["deletelink$ids"];
			...
			$insert_selfttest_details = "insert into SELF_TEST_DETAILS (`PlanCode`,`SelfTestID`,`RowNo`,`MedicalTestID`,`TestName`,`DoctorsName`,`TestDescription`, `Instruction`, `Frequency`,`FrequencyString`,`HowLong`,`HowLongType`,`Link`, `OriginalFileName`,`ResponseRequired`,`StartFlag`,`NoOfDaysAfterPlanStarts`,`SpecificDate`,`CreatedDate`,`CreatedBy`) values ('$plancode_for_current_plan', '$current_selftest_num', '$count','$selftestName', '$selftestOther','$doctor_name','$selftestdesc','$when','$frequency','$frequencystring','$howlong','$howlongtype','$fullpath','$filename','$responserequired','$startflag','$numberofdaysafterplan','$specific_date', now(), '$logged_userid');";
    ...
    <div class="col-sm-2 paddingrl0"  style="display:none;" id="sidebargrid">
      ...
      <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 paddingrl0" align="right"  id="plantitle"><button type="button" class="btns" align="right" id="finished_adding"><img src="images/finishAdd.png" style="height:20px;width:auto;margin-bottom:3px;">&nbsp;<?php if(isset($FINISHADDING) &&  $FINISHADDING !=''){echo $FINISHADDING;} else{echo 'FINISH ADDING';}?></button></div>
    	...
    	<div class="navbar navbar-default" role="navigation" style="height: 100%;background-color:#004F35;position:fixed;width:16.7%;">
    		...
    		<h6 style="font-family:Freestyle;font-size:33px;margin-top:-1px;letter-spacing:1px;color:#f2bd43;background-color:#000;"> <?php if(isset($SelfTestList) &&  $SelfTestList !=''){echo $SelfTestList;} else{echo 'Self Test List :';}?></h6>
    		<div class="sidebarheadings"><?php if(isset($MasterSelfTests) &&  $MasterSelfTests !=''){echo $MasterSelfTests;} else{echo 'Master Self Tests :';}?></div>
    			<div class="panel-group masterplanactivities" id="accordion1" role="tablist" aria-multiselectable="true" style="max-height:250px;overflow:scroll;overflow-x:hidden;">
		    		...
		    		<div class="sidebarheadings" style="margin-top: -13px;"><?php if(isset($AssignedSelfTests) &&  $AssignedSelfTests !=''){echo $AssignedSelfTests;} else{echo 'Assigned Self Tests : ';}?></div>
		...
		$(document).ready(function() {
			...
			$(document).on('change', '.testnameselect', function () {
			   ...
			   var instruction_options = "<?php echo $instruction_options;?>";
			   	instruction_options = "<option style='display:none;' value='0'>select</option>"+instruction_options;
			   if(value == 0){
			   	//alert(1);
			    ...
			    $('#selftestOther'+id).focus();
			   	...
			   	$('#when'+id).html(instruction_options);
			   }else if(value == 5) {
			   	//Postprandial blood sugar
			   	//Populate when (select box)
			   	$('#selftestOther'+id).prop('disabled', true);
			   	$('#selftestOther'+id).css('opacity', '0.2');
			   	$('#doctorName'+id).focus();
			   	$('#when'+id).html("<option style='display:none;' value='0'>select</option><option value='5'>After Breakfast</option><option value='9'>After Lunch</option><option value='18'>After Dinner</option>");
			   }else {
			   	$('#selftestOther'+id).prop('disabled', true);
			   	$('#selftestOther'+id).css('opacity', '0.2');
			   	$('#doctorName'+id).focus();
