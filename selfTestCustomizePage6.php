...
<body>
  <div>
   <form name="frm_plan_selftest" id="frm_plan_selftest" method="post" action="cust_self_new_edited.php">
      ...
      <table id="pdata" class="table">
        <?php
  $get_self = "select UserID, PlanCode, SelfTestID, RowNo, MedicalTestID, TestName, DoctorsName, TestDescription, InstructionID, Frequency, FrequencyString, HowLong, HowLongType,Link, ResponseRequired, StartFlag, NoOfDaysAfterPlanStarts, SpecificDate from USER_SELF_TEST_DETAILS where PlanCode='$plancode' and SelfTestID = '$selfno' and UserID = '$userid'";
  //echo $get_self;exit;
  ...
  while ($selfrow = mysql_fetch_array($get_self_run)) {
    ...
    $Instruction            = $selfrow['InstructionID'];
